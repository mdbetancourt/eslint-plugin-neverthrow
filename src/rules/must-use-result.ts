...
import { AST_NODE_TYPES, TSESLint, ParserServices } from '@typescript-eslint/experimental-utils';
...
function isResultLike(checker: TypeChecker, parserServices: ParserServices, node?: TSESTree.Node | null): boolean {
...
function getAssignation(checker: TypeChecker, parserServices: ParserServices, node: TSESTree.Node): TSESTree.Identifier | undefined {
  if(node.type === AST_NODE_TYPES.VariableDeclarator && isResultLike(checker, parserServices, node.init) && node.id.type === AST_NODE_TYPES.Identifier) {
    return node.id
  }
...
// function isReturned(checker: TypeChecker, parserServices: ParserServices,  node: TSESTree.Node): boolean {
//   if(node.type === AST_NODE_TYPES.ReturnStatement) {
//     return isResultLike(checker, parserServices, node.argument)
//   }
//   if(node.type === AST_NODE_TYPES.BlockStatement) {
//     return false
//   }
//   return isReturned(checker, parserServices, node)
// }
...
function processSelector(
  ...
  if(assignedTo) {
    ...
    if(references.length > 0) {
      return references
        .some(ref => processSelector(context, checker, parserServices, ref.identifier, currentScope.block))
    }
  }
...
const rule: TSESLint.RuleModule<MessageIds, []> = {
  meta: {
    ...
    messages: {
      mustUseResult: "Result must be handle with either of match, unwrapOr or _unsafeUnwrap.",
    },
    schema: [],
    type: 'problem',
  },
